...
module WordPress
  def self.import(database, user, password, table_prefix = "wp", host = 'localhost')
    ...
    %w(_posts _drafts img/posts/featured).each{|folder| FileUtils.mkdir_p folder}
    ...
    db[query].each do |post|
      ...
      slug       = post[:post_name]
      date       = post[:post_date]
      content    = ReverseMarkdown.convert post[:post_content]
      ...
      `wget -O "img/posts/featured/#{image}" "#{post[:post_image]}"` unless File::exists?("img/posts/featured/#{image}") || post[:post_image].nil?
      ...
      db[categories_and_tags_query % post[:ID]].each do |category_or_tag|
        ...
        } unless category_or_tag[:taxonomy] == 'post_format'
      end
